...
async function handleShareToCommunity() {
  ...
  try {
    await copyText(JSON.stringify(shareData, null, 2));
    msg.success("主题已复制到剪贴板，可粘贴到社区分享");
  } catch {
  ...
  >
          ...
          <div class="theme-editor__grid">
            <div v-for="key in group.keys" :key="key" class="theme-editor__color-item">
              ...
              <div class="theme-editor__color-control">
                <div class="theme-editor__color-swatch" :style="{ backgroundColor: colors[key] }" />
                <n-color-picker
                  :value="colors[key]"
                  :modes="['hex']"
                  size="small"
    ...
    <template #footer>
      <div class="theme-editor__footer">
        <n-button size="small" @click="handleShareToCommunity">
          <template #icon>
            <Share2 :size="14" :stroke-width="1.75" />
          </template>
          分享到社区
        </n-button>
        <div class="theme-editor__footer-spacer" />
