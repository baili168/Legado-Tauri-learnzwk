...
export const useReadingStatsStore = defineStore("readingStats", () => {
  ...
  function startSession(bookId: string, bookName: string, chapterIndex: number): string {
    loadSessions();
  ...
  function endSession(sessionId: string, wordsRead: number): void {
    ...
    const now = Date.now();
    session.endTime = now;
    session.durationSecs = Math.max(0, Math.round((now - session.startTime) / 1000));
  ...
  function getBookStats(bookId: string): BookStats {
    ...
    const sorted = [...bookSessions].sort((a, b) => a.startTime - b.startTime);
    const totalDuration = bookSessions.reduce((sum, s) => sum + s.durationSecs, 0);
    const totalWords = bookSessions.reduce((sum, s) => sum + s.wordsRead, 0);
  ...
  function getGlobalStats(): GlobalStats {
    ...
    const totalDuration = allSessions.reduce((sum, s) => sum + s.durationSecs, 0);
    const totalWords = allSessions.reduce((sum, s) => sum + s.wordsRead, 0);
    ...
    const avgDailyMinutes =
      totalDays > 0 ? Math.round((totalDuration / 60 / totalDays) * 10) / 10 : 0;
  ...
  function getDailyHeatmap(year: number, month: number): DailyHeatmapEntry[] {
    ...
    for (const s of sessions.value) {
      ...
      dayMinutesMap.set(s.date, current + Math.round(s.durationSecs / 60));
    }
  ...
  function getBookReadDates(bookId: string): string[] {
    ...
    const dateSet = new Set(
      sessions.value.filter((s) => s.bookId === bookId && s.durationSecs > 0).map((s) => s.date),
    );
    return [...dateSet].sort();
  }
  ...
  function compactSessions(): void {
    ...
    for (const group of groups.values()) {
      ...
      const merged: ReadingSession = {
        ...
        durationSecs: sorted.reduce((sum, s) => sum + s.durationSecs, 0),
    ...
    sessions.value = compacted.sort((a, b) => b.startTime - a.startTime);
    saveSessions();
  }
