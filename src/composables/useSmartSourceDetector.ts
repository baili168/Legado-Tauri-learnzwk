...
const AD_ID_PATTERNS: RegExp[] = [/^(ad|ads|banner|popup|sponsor|promo)/i, /^(gg|guanggao)/i];

const AD_TAG_PATTERNS: RegExp[] = [/^iframe$/i, /^ins$/i];
...
export function generateNovelSource(result: AnalysisResult): string {
  ...
  const bookListItemSel =
    bookItemSel?.selector ||
    bookListSel?.selector ||
    ".result-item, .book-list li, .novel-list li, .search-list li";
  const tocItemSel =
    chapterNameSel?.selector ||
    tocSel?.selector ||
    "#list dd, #chapterlist dd, .chapter-list li, #ul_all_chapters li";
  const contentContainerSel =
    contentSel?.selector ||
    "#content, #chaptercontent, .content, .chapter-content, #articlecontent, .txt, #htmlContent";
  const titleItemSel = titleSel?.selector || 'h1, .title, .book-title, [property="og:title"]';
  const authorItemSel =
    authorSel?.selector || '.author, .book-author, [property="og:novel:author"]';
...
export function generateVideoSource(result: AnalysisResult): string {
  ...
  const videoItemSel =
    bookItemSel?.selector || ".video-item, .movie-item, .video-list li, .module-item";
...
export function generateComicSource(result: AnalysisResult): string {
  ...
  const comicItemSel =
    bookItemSel?.selector || ".comic-item, .cartoon-item, .manga-list li, .comic-list li";
  ...
  const tocItemSel =
    chapterNameSel?.selector ||
    tocSel?.selector ||
    ".chapter-list li, #chapterlist li, .comic-chapter a";
  const contentElSel =
    contentSel?.selector ||
    ".comic-content img, .chapter-img img, .manga-page img, #chapterArea img";
...
export function generateAudioSource(result: AnalysisResult): string {
  ...
  const audioItemSel =
    bookItemSel?.selector || ".audio-item, .album-item, .music-list li, .sound-list li";
  ...
  const tocItemSel =
    chapterNameSel?.selector ||
    tocSel?.selector ||
    ".episode-list li, .track-list li, #playlist li";
...
function buildMockAnalysisResult(url: string): AnalysisResult {
  ...
  return {
    ...
    selectors: [
      {
        purpose: "search",
        selector: 'input[name="searchkey"], input[name="keyword"]',
        confidence: 0.6,
      },
      {
        purpose: "bookItem",
        selector: ".result-item, .book-list li, .novel-list li",
        confidence: 0.5,
      },
      ...
      {
        purpose: "content",
        selector: "#content, #chaptercontent, .content, .chapter-content",
        confidence: 0.5,
      },
  ...
  async function startAnalysis(targetUrl?: string) {
    ...
    try {
      ...
      const analysisResult = await analyzeUrl(
        target,
        state.sampleUrls.length > 0 ? [...state.sampleUrls] : undefined,
      );
